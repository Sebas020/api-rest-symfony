...
class VideoController extends AbstractController
{
                ...
                if(!empty($user_id) && !empty($title)){
                    ...
                    if($id == null){
                        ...
                        $data = [
                            'status' => 'success',
                            'code' => 200,
                            'message' => 'El vídeo se ha creado correctamente.',
                            'video' => $video
                        ];
                    }else{
    ...
    public function videos(Request $request, JwtAuth $jwt_auth, PaginatorInterface $paginator){
        ...
        //Si es válido, 
        if($auth_check){
            //Conseguir la identidad del usuario (objeto)
            $identity = $jwt_auth->checkToken($token, true);
            //Cofigurar el bandle de paginación Documentación (configurar bundles.php, services.yaml)
            ...
            $data = [
                'status' => 'error',
                'code' => 404,
                'message' => 'No se pueden listar los vídeos en estos momentos'
            ];
        }
        return $this->resjson($data);
    }
    ...
    public function video(Request $request, JwtAuth $jwt_auth, $id = null){
        ...
        if($auth_check){
            ...
            $identity = $jwt_auth->checkToken($token, true);
            //Sacar el objeto del vídeo en base al id
            $video = $this->getDoctrine()->getRepository(Video::Class)->findOneBy([
                'id' => $id,
                'user' => $identity->sub
            ]);
            //Comprobar si el vídeo existe y es propuedad del usuario identificado
